...
def _ots_bin() -> str:
    import shutil
    return shutil.which("ots") or str(Path(sys.executable).with_name("ots"))
...
def _check_ots(sha_file: Path) -> str:
    ...
    if not ots_file.exists():
        return "ots:MISSING"
    try:
        r = subprocess.run([_ots_bin(), "verify", str(ots_file)], capture_output=True, text=True, timeout=120)
